defmodule FoxGoose do
  ...
  def solver(lista_actual) do
    ...
    case posi do
      ...
      1 -> elem_en_bote = Enum.at(lista_actual, 1) |> List.delete(:boat)
       lista_actual = List.update_at(lista_actual, 1, &(&1 = [:boat]))
       [List.update_at(lista_actual, 0, &(&1 ++ elem_en_bote)), List.update_at(lista_actual, 2, &(&1 ++ elem_en_bote))]

     2 ->  Enum.at(lista_actual, 2)
       #concatenar el elemento a lista del bote
       |> Enum.map(fn x ->
         List.update_at(lista_actual, 1, &(&1 ++ [x, :you]))
