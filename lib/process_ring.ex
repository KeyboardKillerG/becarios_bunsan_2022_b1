defmodule ProcessRing do
  ...
  def init(n) do
    pids =
      Enum.map(1..n, fn x -> spawn(ProcessRing, :ring_process, [0, List.to_string([x + 64])]) end)
        ...
        {:loop, data} ->
          ...
          if round <= total_rounds do
            if hop < total_hops do
              ...
              send(
                point_to,
                {:loop, %{control: [round, hop + 1, total_rounds, total_hops], msg: msg}}
              )
              ...
              send(
                point_to,
                {:loop, %{control: [round + 1, 1, total_rounds, total_hops], msg: msg}}
              )
